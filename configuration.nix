...
let
  user      = "admin";
  password  = "password";
  hostname  = "68fac9721ec5a2b67f264f63";  # keep your template if you render it; otherwise set a real name
  ...
  systemd.services.polyflow-webrtc = {
    ...
    script = ''
      ...
      if [ -f "${wsDir}/install/setup.bash" ]; then
        ...
        . "${wsDir}/install/setup.bash"
      else
        echo "[webrtc] No install/setup.bash found; did build succeed?" >&2
        exit 1
      fi
